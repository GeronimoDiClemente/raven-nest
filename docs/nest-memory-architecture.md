...
| R-5 | `better-sqlite3` native rebuild breaks the Windows/Mac CI (the repo already fights this with node-pty and `--config.npmRebuild=false`). | Medium | Add to `postinstall` early in Phase 1 and validate on both CI runners before building anything on top. Fallback: `sql.js`/WASM (slower, but no native step). |
