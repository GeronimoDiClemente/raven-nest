...
> **⬜ NO EMPEZADA en el run autónomo (2026-08-15) — decisión deliberada.** Es la épica más grande (L) y la más orientada a **UI + orquestación de git** (D3 = comparar N diffs lado a lado + merge del ganador / discard de perdedores), no lógica pura testeable como C2/agent-status. Ese trabajo necesita validación visual en vivo (imposible desatendido) y bastante presupuesto. **Recomendación de retomada:** empezar por **D1** (multi-issue → multi-worktree), que es "puro integrations" y reusa piezas ya existentes: `ticketBranchName` (branch por ticket, pura), `runWorktreeAdd`/`performWorktreeAdd` (crear worktree), `ticketLoop`/`TicketsBridge` (tickets), y el flujo `startWork`. El núcleo de D1 es orquestación (seleccionar N tickets → `Promise.all` de N worktree-creates bindeados a su ticket) + UI de multi-select — poca lógica pura nueva. D2/D3 (race same-prompt + UI de comparación/merge) son el grueso pesado; hacerlos con más contexto y con Gero para validar la UX del merge/discard.
