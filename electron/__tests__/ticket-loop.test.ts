...
// T8 — Motor 3: señal CI. Con bus, pollOnce consulta los checks del PR abierto y
// emite `ci.failed` cuando el HEAD del branch propio está rojo. Bus-only y
// aditivo: sin bus no hay fetch de check-runs (path H3 verbatim). Sólo GitHub.
describe('TicketLoop señal CI (ci.failed) en pollOnce', () => {
  let provider: TicketProvider
  let loop: TicketLoop
  let bus: EventBus
  ...
  function fakeGithub(pr: unknown, checkRuns: unknown) {
  ...
  it('emite ci.failed cuando un check del PR abierto está en failure', async () => {
    await loop.startWork('jira', ticket, 'gero/PROJ-1-fix', {} as never, 'acme/app')
    const emitSpy = vi.spyOn(bus, 'emit')
    const fetchMock = fakeGithub(
      [{ state: 'open', merged_at: null, head: { sha: 'deadbeef' } }],
      [{ name: 'build', status: 'completed', conclusion: 'failure', html_url: 'https://ci/run/1' }],
    )
    const deps = { getToken: () => 'tok', getConfig: () => ({}), fetch: fetchMock as unknown as typeof fetch }
    await loop.pollOnce('acme/app', deps)
    const ci = emitSpy.mock.calls.filter((c) => (c[0] as DomainEvent).type === 'ci.failed')
    expect(ci).toHaveLength(1)
    expect(ci[0][0]).toMatchObject({ type: 'ci.failed', branch: 'gero/PROJ-1-fix', repoFullName: 'acme/app' })
    // el PR abierto también enrutó pr.opened por el bus (aditivo, no rompe H3)
    expect(emitSpy.mock.calls.some((c) => (c[0] as DomainEvent).type === 'pr.opened')).toBe(true)
  })

  it('NO emite ci.failed cuando los checks del PR pasan', async () => {
    await loop.startWork('jira', ticket, 'gero/PROJ-1-fix', {} as never, 'acme/app')
    const emitSpy = vi.spyOn(bus, 'emit')
    const fetchMock = fakeGithub(
      [{ state: 'open', merged_at: null, head: { sha: 'cafe' } }],
      [{ name: 'build', status: 'completed', conclusion: 'success' }],
    )
    const deps = { getToken: () => 'tok', getConfig: () => ({}), fetch: fetchMock as unknown as typeof fetch }
    await loop.pollOnce('acme/app', deps)
    expect(emitSpy.mock.calls.filter((c) => (c[0] as DomainEvent).type === 'ci.failed')).toHaveLength(0)
  })

  it('no re-emite ci.failed para el mismo commit rojo en ciclos sucesivos (evita spam de notify)', async () => {
    await loop.startWork('jira', ticket, 'gero/PROJ-1-fix', {} as never, 'acme/app')
    const emitSpy = vi.spyOn(bus, 'emit')
    const fetchMock = fakeGithub(
      [{ state: 'open', merged_at: null, head: { sha: 'sha-rojo' } }],
    ...
    await loop.pollOnce('acme/app', deps)
    await loop.pollOnce('acme/app', deps)
    expect(emitSpy.mock.calls.filter((c) => (c[0] as DomainEvent).type === 'ci.failed')).toHaveLength(1)
  })

  it('re-emite ci.failed si un commit NUEVO vuelve a romper el CI', async () => {
    await loop.startWork('jira', ticket, 'gero/PROJ-1-fix', {} as never, 'acme/app')
    const emitSpy = vi.spyOn(bus, 'emit')
    const deps = { getToken: () => 'tok', getConfig: () => ({}), fetch: (() => {}) as unknown as typeof fetch }
    ;(deps as { fetch: unknown }).fetch = fakeGithub(
      [{ state: 'open', merged_at: null, head: { sha: 'sha-1' } }],
      [{ name: 'build', status: 'completed', conclusion: 'failure' }],
    ) as unknown as typeof fetch
    await loop.pollOnce('acme/app', deps)
    ;(deps as { fetch: unknown }).fetch = fakeGithub(
      [{ state: 'open', merged_at: null, head: { sha: 'sha-2' } }],
      [{ name: 'build', status: 'completed', conclusion: 'failure' }],
    ) as unknown as typeof fetch
    await loop.pollOnce('acme/app', deps)
    expect(emitSpy.mock.calls.filter((c) => (c[0] as DomainEvent).type === 'ci.failed')).toHaveLength(2)
  })

  it('sin bus adjunto pollOnce NO consulta check-runs (path H3 verbatim)', async () => {
    const bare = new TicketLoop()
    bare.register('jira', () => provider)
    await bare.startWork('jira', ticket, 'gero/PROJ-1-fix', {} as never, 'acme/app')
    const fetchMock = fakeGithub(
      [{ state: 'open', merged_at: null, head: { sha: 'x' } }],
      [{ name: 'build', status: 'completed', conclusion: 'failure' }],
    )
    const deps = { getToken: () => 'tok', getConfig: () => ({}), fetch: fetchMock as unknown as typeof fetch }
    await bare.pollOnce('acme/app', deps)
